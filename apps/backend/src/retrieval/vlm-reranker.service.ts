...
export class VlmRerankerService {
  ...
  async rerank(
    ...
    const poolLimit = this.config.vlmConcurrency || 5;
  ...
  private async evaluateCandidate(
    ...
    try {
      ...
      // Relevance score is 0 - 100. Normalize around 50:
      // score = 100 -> multiplier = 1 + (50/50)*weight = 1 + weight (e.g. 1.6x)
      // score = 50  -> multiplier = 1.0x
      // score = 0   -> multiplier = 1 - (50/50)*weight = 1 - weight (e.g. 0.4x)
