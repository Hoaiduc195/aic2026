...
describe('Postgres retrieval branches', () => {
  it('uses parameterized FTS and maps caption evidence to a candidate', async () => {
    const db = database([{
      ...
      original_frame_id: 42, keyframe_no: 3, start_ms: 1000, end_ms: 2000,
  ...
  it('returns exact object matches without running fuzzy matching', async () => {
    const db = database([{
      ...
      original_frame_id: 5, keyframe_no: 12, start_ms: 100, end_ms: 101,
    ...
    expect(result.candidates[0]).toMatchObject({ keyframe_no: 12, matched_terms: ['bicycle'] });
    ...
    expect(sql).toContain('f.keyframe_no');
