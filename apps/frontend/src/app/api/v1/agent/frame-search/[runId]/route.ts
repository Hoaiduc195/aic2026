...
export async function GET(_request: NextRequest, context: RouteContext) {
  ...
  try {
    const upstream = requestBackend(`/v1/agent/frame-search/${encodeURIComponent(runId)}`);
    if (!upstream) return NextResponse.json({ message: 'Backend chưa được cấu hình.' }, { status: 503 });
    return await forwardJsonResponse(await upstream, 'Không thể tải trạng thái agent run.');
  } catch {
