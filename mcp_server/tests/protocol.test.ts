...
describe('MCP stdio protocol', () => {
  it('lists AIC tools and serves an exact frame call', async () => {
    ...
    expect(listed.tools.map((tool) => tool.name)).toEqual(expect.arrayContaining([
      ...
      'suggest_vqa_answer', 'get_candidates', 'get_selection', 'preview_submission', 'parse_submission_csv', 'search_loop', 'get_search_session', 'check_trake_sequence', 'trace_answer',
    ]));
...
function mockBackend(request: IncomingMessage, response: ServerResponse): void {
  ...
  if (request.url === '/v1/videos/v-1/frames/10') {
